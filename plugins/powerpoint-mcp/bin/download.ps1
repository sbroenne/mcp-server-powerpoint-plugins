...
$PluginName = "powerpoint-mcp"
$ExecutableName = "mcp-powerpoint.exe"
$RepoOwner = "sbroenne"
...
function Get-State {
    ...
    try {
        ...
        Write-Status "[powerpoint-mcp] Ignoring unreadable bootstrap state and starting fresh." "DarkYellow"
        return New-State
    }
}
...
function Get-LatestReleaseMetadata {
    Write-Status "[powerpoint-mcp] Checking latest GitHub release..." "Cyan"

    try {
        $headers = @{
            Accept       = "application/vnd.github+json"
            "User-Agent" = "powerpoint-mcp-plugin-bootstrap"
        }
        ...
        try {
            ...
            $exception = [System.InvalidOperationException]::new(
                "Failed to resolve the latest PowerPointMcp MCP server release. Failed to query the GitHub release API. $($_.Exception.Message)`nRelease page: $ReleasePageUrl",
                $_.Exception)
            $exception.Data["AllowCachedFallback"] = $true
            throw $exception
        }
        $releaseVersion = $release.tag_name -replace '^v', ''
        $assetName = "PowerPointMcp-MCP-Server-$releaseVersion-windows.zip"
        ...
        throw "Failed to resolve the latest PowerPointMcp MCP server release. $_`nRelease page: $ReleasePageUrl"
    }
}
    ...
    try {
        ...
        if (Test-Path $ReleaseDir) {
            ...
            if ($null -ne $installed -and (Test-FileLocked -Path $installed.FullName)) {
                Write-StatusError "[powerpoint-mcp] $ExecutableName is in use and cannot be replaced right now; keeping the existing install."
...
function Ensure-LatestRuntime {
    ...
    if ([string]::IsNullOrWhiteSpace($State.assetName) -or
        ...
        throw "powerpoint-mcp must download the runtime but has no verified release metadata available. Check your network connection and try again.`nRelease page: $ReleasePageUrl"
    }
    ...
    try {
        ...
        for ($attempt = 1; $attempt -le 2; $attempt++) {
            try {
                ...
                if ($downloadRequired) {
                    Write-Status "[powerpoint-mcp] Downloading $($State.assetName)..." "Yellow"
                    Save-RuntimeArchive -Uri $State.assetUrl -DestinationPath $downloadZipPath -ExpectedSha256 $State.expectedSha256
                } else {
                    Write-Status "[powerpoint-mcp] Reusing cached package $($State.assetName)." "DarkGray"
                }

                Write-Status "[powerpoint-mcp] Extracting $($State.assetName)..." "Yellow"
                ...
                if ($attempt -lt 2) {
                    Write-StatusError "[powerpoint-mcp] Runtime install failed: $($_.Exception.Message)"
                    Write-StatusError "[powerpoint-mcp] Retrying once with a fresh download."
                }
            }
        }
...
if ($env:OS -ne "Windows_NT") {
    throw "powerpoint-mcp plugin bootstrap is Windows-only."
}
...
if ($sessionNeedsFreshnessCheck) {
    try {
        ...
        Write-StatusError "[powerpoint-mcp] Could not check for updates: $($_.Exception.Message)"
        Write-StatusError "[powerpoint-mcp] Continuing with the cached runtime $($state.latestTag)."
    ...
    Write-Status "[powerpoint-mcp] Freshness already checked for this Copilot session." "DarkGray"
}
...
Write-Status
Write-Status "✅ PowerPointMcp MCP runtime ready." "Green"
